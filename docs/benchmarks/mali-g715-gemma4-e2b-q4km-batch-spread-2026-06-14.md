...
is the important change, and `-b 256 -ub 64` is the strongest row in this
Gemma 4 E2B spread.
...
```sh
-ngl 99 -t 2 -fa 0 -b 256 -ub 64
```

Compared with `128/32`, `256/64` is about 3.4x faster in prompt processing
(`31.36` vs `9.18` PP tok/s), about 1.9x faster in token generation (`7.34` vs
`3.93` TG tok/s), and about 2.9x faster wall-clock (`71s` vs `204s`).

Compared with `128/64`, `256/64` has similar prompt-processing throughput but
much better decode throughput (`7.34` vs `3.97` TG tok/s). That makes `256/64`
the better default for the interactive Pi session with Gemma 4 E2B.
...
- The Ternary-Bonsai `128/64` and `256/64` result should not be reused for
  Gemma. In this Gemma run, `256/64` is materially better.
