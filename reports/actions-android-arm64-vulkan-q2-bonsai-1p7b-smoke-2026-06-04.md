...
```sh
./llama-bench \
  -m /path/to/Ternary-Bonsai-1.7B-Q2_0.gguf \
  -p 64 -n 64 -r 1 \
  -dev Vulkan0 -ngl 99
```
...
This fills the smaller-model benchmark slot. PrismML does not currently publish
a 2B Bonsai GGUF in this series; the closest smaller PrismML Bonsai GGUF found
during this run was the 1.7B `Q2_0` model.
